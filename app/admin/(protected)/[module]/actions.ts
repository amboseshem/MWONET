...
export async function updateMemberStatus(formData:FormData){
 const id=text(formData,"id");const status=text(formData,"status") as "PENDING"|"ACTIVE"|"INACTIVE"|"SUSPENDED"|"TERMINATED";if(!id||!status)return;
 const existing=await db.memberProfile.findUnique({where:{id},include:{user:true}});if(!existing)return;
 const memberNumber=status==="ACTIVE"?(existing.memberNumber||`MWO-${new Date().getFullYear()}-${existing.id.slice(-6).toUpperCase()}`):existing.memberNumber;
 const row=await db.$transaction(async tx=>{
  const updated=await tx.memberProfile.update({where:{id},data:{status,memberNumber,joinedAt:status==="ACTIVE"?(existing.joinedAt||new Date()):existing.joinedAt}});
  if(status==="SUSPENDED"||status==="TERMINATED")await tx.user.update({where:{id:existing.userId},data:{status:"SUSPENDED"}});
  if(status==="ACTIVE")await tx.user.update({where:{id:existing.userId},data:{status:"ACTIVE"}});
  return updated;
 });
 await audit("member.status","MemberProfile",row.id,{status,memberNumber});refresh("members");revalidatePath("/portal");
}
